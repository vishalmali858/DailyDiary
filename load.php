...
<script> $( "#Brief" ).load( "$name" ); </script></head>
<body style=background-image:url("l1.jpg")>
  ...
  <div class="modal-dialog">
	...
	<b><font color="blue" size="5" face="verdana">YOUR DIARY:</font></b>
      </div>
...
<?php
			if($flag1=="0")
			{
				...
				$date=date('d-m-Y',strtotime($dateb));
				$datef=date('Y-m-d',strtotime($dateb));
				$name1="uploads/".$id.'_'.$datef;
				$name1=$name1.'.txt';
				$homepage = file_get_contents($name1);
				echo "<center><font color=red size=5>$dateb<font color=black size=3>(TO DOWNLOAD CLICK ON THE DIARY)</font></center>";
				echo "<a href=$name1 download><textarea style=width:100%; readonly>$homepage</textarea></a>";
				echo "<center>";
				echo "<a href=home.php><input type=button value=BACK></a>&nbsp &nbsp";
				echo "<a href=a.php><input type=button value=ATTACHMENT></a></center>";
				exit();
			}
			else if($flag1=="3")
			{
...
?>
		...
		<center>
			...
			<a href="a.php"><input type="button" value="ATTACHMENT"></a>
		</center>
	</form>
      </div>
